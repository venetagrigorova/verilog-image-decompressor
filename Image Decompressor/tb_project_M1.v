...
logic M1_start, M1_stop;
logic [5:0] milestone1_state_debug;

integer num_correct, num_error;
...
initial begin
	...
	//wait (uut.top_state == 3);	//this assumes S_IDLE is the first in the list where the states are enumerated
...
//	wait (uut.top_state == 1);	//this assumes we go back to S_IDLE when we are done
  #250;
  $write("I am in top state %s at time %t and timer is %d\n", uut.top_state, $realtime, uut.UART_timer);
	wait (uut.top_state == S_IDLE);	//this assumes we go back to S_IDLE when we are done
  $write("Now I am in top state %s at time %t\n", uut.top_state, $realtime);
...
always @ (posedge Clock_50) begin
	if (uut.SRAM_we_n == 1'b0) begin	//signal names within project (instantiated as uut) should match here, assuming names from experiment4a
		...
		if (SRAM_ARRAY[uut.SRAM_address][15:8] != uut.SRAM_write_data[15:8]) begin
			$write("error: wrote value %d (%x hex) to location %d (%x hex), should be value %d (%x hex)\n\n",
				uut.SRAM_write_data[15:8], uut.SRAM_write_data[15:8], uut.SRAM_address, uut.SRAM_address,
				SRAM_ARRAY[uut.SRAM_address][15:8], SRAM_ARRAY[uut.SRAM_address][15:8]);
			//$write("sim time %t\n", $realtime);
		...
		if (SRAM_ARRAY[uut.SRAM_address][7:0] != uut.SRAM_write_data[7:0]) begin
			$write("error: wrote value %d (%x hex) to location %d (%x hex), should be value %d (%x hex)\n\n",
				uut.SRAM_write_data[7:0], uut.SRAM_write_data[7:0], uut.SRAM_address, uut.SRAM_address,
				SRAM_ARRAY[uut.SRAM_address][7:0], SRAM_ARRAY[uut.SRAM_address][7:0]);
			//$write("sim time %t\n", $realtime);
			//$write("print some useful debug info here...\n");
		//	$write("m1 state %d\n", uut.m1.state);
		//	$write("...or take a look at the last few clock cycles in the waveforms that lead up to this error\n");
		...
		/*if (SRAM_ARRAY[uut.SRAM_address] == uut.SRAM_write_data) begin
			$write("correct: wrote value %d (%x hex) to location %d (%x hex), should be value %d (%x hex)\n\n",
